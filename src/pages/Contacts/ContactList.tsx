...
const relationshipTone = (relationship: string) => {
  ...
  if (value === 'not connected') return 'border-slate-200 bg-slate-50 text-slate-500';
  ...
  <div className={groupMode === 'NONE' ? '' : 'space-y-6'}>
    {groups.map(([group, items]) => (
      <section key={group}>
        ...
        <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_16px_40px_-36px_rgba(15,23,42,0.7)]">
          <div
            className={`hidden items-center gap-5 border-b border-slate-200 bg-slate-50/70 px-5 py-2.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-400 sm:grid ${columnGrid}`}
            aria-hidden="true"
          >
            <span>Person</span>
              ...
              return (
                ...
                >
                  <span className="flex min-w-0 items-center gap-3">
                    <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-slate-200 bg-slate-50 text-xs font-semibold tracking-wide text-slate-600 shadow-sm">
                  ...
                  <RightOutlined className="hidden text-[10px] text-slate-300 transition group-hover:translate-x-0.5 group-hover:text-slate-500 sm:block" />

                  <span className="grid min-w-0 grid-cols-2 gap-3 border-t border-slate-100 pt-3 sm:hidden">
                    <span className="min-w-0">
