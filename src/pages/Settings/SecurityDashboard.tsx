...
const SecurityDashboard: React.FC = () => {
  ...
  const checks = useMemo<CheckItem[]>(() => {
    ...
    return [
      ...
      },
      {
        ...
        detail: data.environment.admin_enabled
      ...
      },
      {
        ...
        detail: data.environment.secure_ssl_redirect
          ? 'HTTP requests redirect to HTTPS.'
          : 'HTTPS redirect is off.',
        status: boolStatus(data.environment.secure_ssl_redirect),
      },
      {
        label: 'Secure cookies',
        detail:
          data.environment.session_cookie_secure && data.environment.csrf_cookie_secure
      ...
      },
      {
        label: 'HSTS',
        detail:
          data.environment.hsts_seconds > 0
      ...
      },
      {
        ...
        status: data.google.oauth_configured
          ? data.google.oauth_connected
            ? 'ok'
            : 'warn'
          : 'bad',
  ...
  const score = useMemo(() => {
    ...
    return checks.reduce((acc, item) => ({ ...acc, [item.status]: acc[item.status] + 1 }), {
      ok: 0,
      warn: 0,
      bad: 0,
    });
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Panel icon={<LockOutlined />} title="Auth & Access">
          <Info
            label="Public signup"
            value={data.environment.public_signup_enabled ? 'Enabled' : 'Disabled'}
          />
          ...
          <Info
            ...
            value={data.auth.refresh_rotation_enabled ? 'Enabled' : 'Disabled'}
          />
        </Panel>
        ...
        <Panel icon={<GoogleOutlined />} title="Google Sheets">
          ...
          <Info
            label="Spreadsheet picker"
            value={data.google.can_list_spreadsheets ? 'Ready' : 'Needs reconnect'}
          />
          <Info
            label="Sync configs"
            value={`${data.google.enabled_syncs} enabled / ${data.google.total_syncs} total`}
          />
          <Info label="Last sync" value={formatDateTime(data.google.latest_synced_at)} />
          {data.google.latest_error && (
            <Info label="Latest error" value={data.google.latest_error} danger />
          )}
        </Panel>
        ...
        <Panel icon={<CloudServerOutlined />} title="Deployment">
          ...
          <Info
            ...
            value={String(data.environment.csrf_trusted_origins_count)}
          />
          <Info
            label="Vercel runtime"
            value={data.waf.vercel_project ? 'Detected' : 'Not detected'}
          />
        </Panel>

        <Panel icon={<ThunderboltOutlined />} title="Edge Protection">
          <Info
            label="Scanner path denies"
            value={data.waf.edge_scanner_denies_deployed ? 'Deployed' : 'Unknown'}
          />
          <Info
            label="Bot protection"
            value={data.waf.bot_protection_configured ? 'Configured' : 'Unknown'}
          />
          <Info
            label="AI bot blocking"
            value={data.waf.ai_bots_blocked ? 'Configured' : 'Unknown'}
          />
