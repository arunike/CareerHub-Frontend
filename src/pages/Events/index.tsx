...
const Events = () => {
  ...
  const filteredEvents = filterByYear(events, selectedYear, 'date').filter(event => {
      ...
      if (dateRange && dateRange[0] && dateRange[1]) {
          ...
          if (eventDate.isBefore(dateRange[0], 'day') || eventDate.isAfter(dateRange[1], 'day')) return false;
      }
      return true;
  }).sort((a, b) => {
      let comparison = 0;
      if (sortBy === 'date') {
          ...
          // Duration
          const getDiff = (e: Event) => dayjs(`${e.date}T${e.end_time}`).diff(dayjs(`${e.date}T${e.start_time}`));
          ...
          if (editingId) {
             ...
             if (existing?.is_virtual && existing.parent_event) {
                 Modal.confirm({
                     ...
                     content: 'This is an instance of a recurring event. Do you want to update the entire series?',
                     onOk: async () => {
  ...
  const toggleLock = async (event: Event) => {
      try {
          ...
          setEvents(prev => prev.map(e => e.id === event.id ? {...e, is_locked: !e.is_locked} : e));
  ...
  return (
    <>
                    ...
                    {filteredEvents.map((event) => (
                        <div key={event.id} style={{ height: '100%' }}>
                            <Card 
                                ...
                                title={
                                    <Space>
                                        <Text strong ellipsis style={{ maxWidth: 180 }} title={event.name}>{event.name}</Text>
                                ...
                                extra={
                                    <Space>
                                         ...
                                         <Popconfirm 
                                            ...
                                            description={event.is_virtual ? "Delete options for series?" : "Are you sure?"}
                                         >
                                             <Button 
                                                type="text" 
                                                danger 
                    ...
                    children: (
                        <>
                                ...
                                <Select 
                                    ...
                                    dropdownRender={(menu) => (
                                        ...
                                        <Space style={{ padding: '0 8px 4px' }}>
                                            ...
                                            <Button type="text" icon={<PlusOutlined />} onClick={async () => {
                                                if (!newCategoryName) return;
                                                await createCategory({ name: newCategoryName, color: '#1890ff', icon: newCategoryIcon });
                                                fetchCategories();
                                                setNewCategoryName('');
                            ...
                            <Form.Item label="Recurrence">
                                ...
                                {recurrenceRule && <Button type="text" danger onClick={() => setRecurrenceRule(null)}>Clear</Button>}
                            </Form.Item>
      ...
      <Modal
          ...
          footer={[
              <Button key="close" onClick={() => setViewingEvent(null)}>Close</Button>,
      ...
      >
          {viewingEvent && (
              <Space direction="vertical" size="middle" style={{ width: '100%' }}>
                  <Row>
                      <Col span={12}><Text type="secondary">Date</Text><div>{dayjs(viewingEvent.date).format('MMMM D, YYYY')}</div></Col>
                      <Col span={12}><Text type="secondary">Time</Text><div>{viewingEvent.start_time} - {viewingEvent.end_time}</div></Col>
                  </Row>
                  {viewingEvent.meeting_link && (
                      <div><Text type="secondary">Meeting</Text><div><a href={viewingEvent.meeting_link} target="_blank" rel="noreferrer">{viewingEvent.meeting_link}</a></div></div>
                  )}
                  {viewingEvent.notes && (
                       <div><Text type="secondary">Notes</Text><div style={{ whiteSpace: 'pre-wrap' }}>{viewingEvent.notes}</div></div>
                  )}
              </Space>
          )}
      </Modal>
