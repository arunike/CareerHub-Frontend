...
const Holidays = () => {
  ...
  const handleAdd = async (values: any) => {
    ...
    try {
        const promises = datesToAdd.map(date => createHoliday({
            date,
            description,
  ...
  const toggleLock = async (holiday: Holiday) => {
      try {
          ...
          setHolidays(prev => prev.map(h => h.id === holiday.id ? { ...h, is_locked: !h.is_locked } : h));
        ...
        <Space direction="vertical" size="large" style={{ width: '100%' }}>
                ...
                >
                    <Row gutter={16}>
                        ...
                        <Col span={24} md={8} style={{ display: 'flex', alignItems: 'center' }}>
                             <Form.Item name="is_recurring" valuePropName="checked" style={{ marginBottom: 0, marginTop: 24 }}>
                                 <Checkbox>Recurring (Yearly)</Checkbox>
                             </Form.Item>
                             <Button type="primary" htmlType="submit" icon={<PlusOutlined />} style={{ marginLeft: 'auto', marginTop: 24 }}>
                                 Add
                             </Button>
                        </Col>
                    </Row>
            ...
            >
                <List
                    ...
                    renderItem={(item) => (
                        <List.Item
                            actions={[
                                ...
                                <Popconfirm title="Delete?" onConfirm={() => handleDelete(item.id)} disabled={item.is_locked}>
                                    <Button type="text" danger icon={<DeleteOutlined />} disabled={item.is_locked} />
                                </Popconfirm>
                            ]}
                        >
                            <List.Item.Meta
                                avatar={<CalendarOutlined style={{ fontSize: 20, color: '#1890ff', marginTop: 8 }} />}
                                title={
                                    <Space>
                                        <Text strong>{dayjs(item.date).format('YYYY-MM-DD')}</Text>
                                        {item.is_recurring && <Tag color="blue" icon={<SyncOutlined />}>Yearly</Tag>}
                    ...
                    locale={{ emptyText: <Empty description="No custom holidays found" image={Empty.PRESENTED_IMAGE_SIMPLE} /> }}
                />
            </Card>
        </Space>
      ),
            ...
            <Card>
                <Space align="start">
                    ...
                    <div>
                         ...
                         <div><Text type="secondary">These are automatically excluded from availability. You don't need to add them.</Text></div>
                    </div>
                </Space>
            </Card>
            <List
