...
export default function PublicFaq() {
  return (
    <section
      id="faq"
      className="scroll-mt-28 border-y border-slate-200 bg-white px-5 py-20 sm:px-8 sm:py-28 lg:px-10"
    >
      <div className="mx-auto grid w-full max-w-7xl gap-12 lg:grid-cols-[minmax(260px,0.58fr)_minmax(0,1.42fr)] lg:items-start lg:gap-20">
        <div className="lg:sticky lg:top-28">
          <h2 className="max-w-[12ch] text-balance text-4xl font-bold leading-[1.05] tracking-[-0.035em] text-slate-950 sm:text-5xl">
            The practical questions.
          </h2>
          <p className="mt-6 max-w-[42ch] text-base leading-8 text-slate-600">
        ...
        <div className="border-t border-slate-300">
          {questions.map((item, index) => (
            <details key={item.question} className="group border-b border-slate-200">
              <summary className="grid cursor-pointer list-none grid-cols-[28px_minmax(0,1fr)_40px] items-center gap-3 py-5 text-left outline-none transition-colors duration-200 hover:text-blue-800 focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blue-500 sm:grid-cols-[36px_minmax(0,1fr)_44px] sm:gap-5 sm:py-6 [&::-webkit-details-marker]:hidden">
                <span className="text-xs font-semibold tabular-nums text-slate-400">
                  {String(index + 1).padStart(2, '0')}
                </span>
                <span className="text-base font-semibold leading-6 tracking-[-0.012em] text-slate-950 transition-colors group-open:text-blue-800 sm:text-lg">
                  {item.question}
                </span>
                <span className="flex h-10 w-10 items-center justify-center rounded-full text-blue-700 transition-colors duration-200 group-hover:bg-blue-50 group-open:bg-blue-50 group-open:text-blue-800 sm:h-11 sm:w-11">
              ...
              <div className="grid grid-cols-[28px_minmax(0,1fr)_40px] gap-3 pb-7 sm:grid-cols-[36px_minmax(0,1fr)_44px] sm:gap-5 sm:pb-8">
                <p className="col-start-2 max-w-[68ch] text-[15px] leading-[1.8] text-slate-600">
                  {item.answer}
                </p>
              </div>
            </details>
