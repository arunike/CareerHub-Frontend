...
export default function WeekAtGlance() {
  return (
    <section className="relative overflow-hidden bg-white px-5 py-20 sm:px-8 sm:py-28 lg:px-10">
      <div
        className="pointer-events-none absolute -left-24 top-1/2 h-72 w-72 -translate-y-1/2 rounded-full bg-blue-50 blur-3xl"
        aria-hidden
      />
      <div className="relative mx-auto grid w-full max-w-7xl gap-12 lg:grid-cols-[minmax(0,0.76fr)_minmax(520px,1.24fr)] lg:items-center lg:gap-20">
        <div>
          ...
          <p className="mt-6 max-w-[58ch] text-lg leading-8 text-slate-600">
        ...
        <div className="overflow-hidden rounded-2xl bg-slate-50 shadow-[0_30px_90px_-56px_rgba(30,64,175,0.5)] ring-1 ring-slate-200">
          <div className="flex items-center justify-between border-b border-slate-200 bg-white px-5 py-4 sm:px-6">
            <div>
              <p className="text-sm font-semibold text-slate-950">Week at a glance</p>
              <p className="mt-0.5 text-xs text-slate-500">Illustrative schedule</p>
            </div>
          ...
          <div className="divide-y divide-slate-200">
            {weekItems.map((item) => {
              ...
              return (
                <article
                  key={item.label}
                  className="group grid gap-4 px-5 py-5 transition-colors duration-300 hover:bg-white sm:grid-cols-[44px_minmax(120px,0.55fr)_minmax(0,1.45fr)] sm:items-center sm:px-6"
                >
                  <span
          ...
          <p className="border-t border-slate-200 bg-white px-5 py-3 text-xs leading-5 text-slate-500 sm:px-6">
