...
import { PanelSkeleton } from '../feedback/PageState';
  ...
  <div className="space-y-6 lg:col-span-3">
    <div className="rounded-2xl border border-slate-200 dark:border-white/[0.08] bg-white dark:bg-ink-900 p-5 shadow-[0_18px_48px_-42px_rgba(15,23,42,0.55)] sm:p-8">
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="flex items-center gap-2 text-lg font-bold text-slate-900 dark:text-ink-50">
          <CalendarOutlined className="text-blue-500 dark:text-blue-400" />
          {manageAction === 'cancel' ? 'Booking Time' : '1. Select Date & Time'}
        </h2>
        <div className="flex gap-1 rounded-xl border border-slate-200 dark:border-white/[0.08] bg-slate-50 dark:bg-ink-900 p-1">
          <button
            type="button"
            onClick={() => setSlotView('list')}
            className={`min-h-11 rounded-lg px-4 text-sm font-semibold transition-colors sm:min-h-9 ${
              slotView === 'list'
                ? 'bg-white dark:bg-ink-900 text-blue-700 dark:text-blue-300 shadow-sm'
                : 'text-slate-600 dark:text-ink-200 hover:text-slate-900'
            }`}
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          ...
          <input
            ...
            className="min-h-11 w-full rounded-xl border border-slate-300 dark:border-white/[0.12] bg-white dark:bg-ink-900 px-4 text-sm font-medium transition-colors focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20"
          />
        </div>
        <div className="space-y-2">
          <label
      ...
      {manageAction === 'cancel' ? (
        <div className="rounded-xl border border-slate-200 dark:border-white/[0.08] bg-slate-50 dark:bg-ink-900 p-4 text-sm font-semibold text-slate-700 dark:text-ink-100">
          {managedBooking
        ...
        <div className="space-y-6">
                    ...
                    <button
                      ...
                      className={`flex min-h-16 w-14 snap-start flex-col items-center justify-center rounded-xl border py-3 transition-colors sm:w-auto ${
                        isSelected
                          ...
                          : hasSlots
                            ? 'border-slate-200 dark:border-white/[0.08] bg-white dark:bg-ink-900 text-slate-700 dark:text-ink-100 hover:border-blue-300 hover:bg-blue-50'
                            : 'cursor-not-allowed border-transparent bg-slate-50 dark:bg-ink-900 text-slate-400 dark:text-ink-500 opacity-60'
                      }`}
                      aria-pressed={isSelected}
                      aria-label={`${day.day_name}, ${formatDateOnly(day.date)}${hasSlots ? '' : ', unavailable'}`}
                    >
                      <span
                        className={`mb-1 text-xs font-medium ${isSelected ? 'text-blue-100' : 'text-slate-500 dark:text-ink-400'}`}
                      >
          ...
          <div className="border-t border-slate-200 dark:border-white/[0.08] pt-4">
            {!selectedDay || selectedDay.slots.length === 0 ? (
              <div className="rounded-xl border border-dashed border-slate-300 dark:border-white/[0.12] bg-slate-50 dark:bg-ink-900 px-4 py-10 text-center">
              ...
              {selectedDay.slots.map((slot) => {
                ...
                return (
                  ...
                  >
                    ...
                    <ClockCircleOutlined
                      className={active ? 'text-blue-200' : 'text-blue-500 dark:text-blue-400'}
                    />
                  </button>
                );
              })}
