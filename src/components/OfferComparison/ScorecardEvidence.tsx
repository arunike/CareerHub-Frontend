...
import {
  getEquityLiquidityCopy,
  getRealizableEquity,
} from '../../utils/OfferComparison/equityLiquidity';
import { formatCurrency } from '../../utils/OfferComparison/decisionScoring';
...
const ScorecardEvidence = ({ row }: Props) => (
  <div className="@container border-t border-slate-100 dark:border-white/[0.07] bg-slate-50/40 dark:bg-ink-900/40">
    <div className="grid grid-cols-1 @[30rem]:grid-cols-3">
      ...
      <div className="border-b border-slate-100 dark:border-white/[0.07] px-4 py-3 @[30rem]:border-r @[30rem]:border-b-0 @[30rem]:px-5">
        ...
        <p className="mt-1 text-base font-bold text-slate-900 dark:text-ink-50">
          ...
          <span className="ml-1 text-[10px] font-medium text-slate-400 dark:text-ink-500">
            / yr
          </span>
        </p>
