...
import {
  ...
  VISA_OVERLAY_WEIGHT,
  clamp,
} from '../../utils/OfferComparison/decisionScoring';
import type { CategoryKey } from '../../utils/OfferComparison/decisionScoring';
import type { MaritalStatus } from '../../utils/OfferComparison/offerTypes';
...
const ScorecardSidebar = ({
  ...
  <aside className="flex h-fit flex-col gap-6 lg:sticky lg:top-6">
    {/* Simulation Settings */}
    <div className="rounded-3xl border border-slate-200 dark:border-white/[0.08] bg-slate-50/50 dark:bg-ink-900/50 p-6 shadow-sm">
      <div className="mb-4 flex items-center gap-3">
        <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-emerald-100 dark:bg-emerald-500/15 text-emerald-600 dark:text-emerald-300">
          <svg
            width="16"
            height="16"
            viewBox="0 0 24 24"
        ...
        <div>
          <h3 className="text-sm font-bold text-slate-900 dark:text-ink-50">Offer Simulations</h3>
          <p className="text-[10px] font-medium uppercase tracking-wider text-slate-500 dark:text-ink-400">
            "What-If" Scenarios
          </p>
        </div>
      </div>
      <div className="space-y-4">
        <div>
          <label className="mb-1.5 block text-xs font-semibold text-slate-700 dark:text-ink-100">
            Tax Marital Status
          </label>
          <Select
            value={maritalStatus}
        ...
        <div className="flex flex-col gap-2 pt-2">
          ...
          <button
            ...
            className="flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-white dark:bg-ink-900 px-3 py-2 text-sm font-semibold text-slate-700 dark:text-ink-100 shadow-sm ring-1 ring-inset ring-slate-300 dark:ring-white/[0.12] transition-colors hover:bg-slate-50"
          >
            Save Settings
          </button>
        </div>
      ...
      <div className={clsx('flex items-center gap-2', isWeightsExpanded ? 'mb-6' : 'mb-0')}>
        ...
        >
          <div className="flex min-w-0 items-center gap-3">
            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-sky-100 dark:bg-sky-500/15 text-sky-600 dark:text-sky-300">
              <svg
                width="16"
                height="16"
                viewBox="0 0 24 24"
            ...
            <div className="min-w-0">
              <h3 className="text-sm font-bold text-slate-900 dark:text-ink-50">Score Weights</h3>
              <p className="text-[10px] font-medium uppercase tracking-wider text-slate-500 dark:text-ink-400">
                Current Distribution
              </p>
            </div>
          </div>
          <div className="shrink-0 text-slate-400 dark:text-ink-500" aria-hidden="true">
            {isWeightsExpanded ? (
        ...
        <HelpTooltipTrigger
          ...
          title={
            <div className="space-y-1.5 p-1 text-[11px] leading-relaxed text-slate-200">
              ...
              <p className="border-t border-slate-700 pt-1 text-[10px] text-slate-400 dark:text-ink-500">
        ...
        (() => {
          ...
          return (
            <div id="score-weights-panel" className="space-y-3">
              {/* Total indicator */}
              <div className="rounded-2xl border border-slate-200 dark:border-white/[0.08] bg-white dark:bg-ink-900 px-3 py-2.5 shadow-sm">
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-ink-500">
                    Total Weight
                  </span>
                ...
                {isOver && (
                  <p className="mt-1.5 text-[10px] text-rose-500 dark:text-rose-400 font-medium">
                  ...
                  return (
                    <div
                      ...
                      className={clsx(
                        ...
                        isOver
                          ? 'border-rose-200 dark:border-rose-500/25 bg-rose-50/30 dark:bg-rose-500/10'
                          : 'border-slate-200 dark:border-white/[0.08] bg-white dark:bg-ink-900'
                      )}
                    >
                      <div className="flex flex-col">
                        <span className="text-xs font-bold text-slate-700 dark:text-ink-100">
                          {CATEGORY_LABELS[key]}
                        </span>
                        {isOptional && (
                      ...
                      <UnitNumberInput
                        ...
                        className={clsx(
                          ...
                          isOver
                            ? '[&_.ant-input-number]:!bg-rose-50 dark:!bg-rose-500/10 [&_.ant-input-number-input]:!text-rose-600 dark:!text-rose-300'
                            : '[&_.ant-input-number]:!bg-sky-50 dark:!bg-sky-500/10 [&_.ant-input-number-input]:!text-sky-700 dark:!text-sky-300'
                        )}
                        controls={false}
                      />
                    </div>
              ...
              {anyImmigrationSignal && (
                <div className="rounded-2xl border border-amber-100 dark:border-amber-500/20 bg-amber-50 dark:bg-amber-500/10 px-3 py-2.5">
