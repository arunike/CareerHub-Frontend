...
- **Proration shows its arithmetic.** It read `This role covered 59% of 2025`, which states the answer and hides the sum. It now reads `Held 214 of 365 days in 2025, so $25,050.00 × 58.6% = $14,686.85`, with the day counts coming from `prorationDetail` rather than being re-derived in the component. Switched off, it says how many days were worked anyway, so the number you are declining to prorate is still visible
...
- **`Contributed in {year}` is a subtotal, and says so.** The block listed the year's projected traditional, Roth, match and total — the same $7,139.38 that made the gain read as a loss — with nothing marking it as a forecast. Every line is now the amount actually paid in, qualified in the header with `through Aug 28 · 18 of 26 paychecks`, and the projection moves to a quiet line beneath it (`On the current rate the year finishes at $12,942.22`). The qualifier disappears once the last paycheck of the year has been paid, so a finished year reads as a plain total. The limit bar above it stays a forecast — it is labelled `At this rate you finish the year…` and answers a different question
