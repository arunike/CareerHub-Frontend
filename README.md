...
- **Analytics tab in the URL**: `/analytics?tab=career` drives which tab is open, so a refresh, the back button, or a shared link all land on the tab you were looking at rather than snapping back to Availability. Switching tabs replaces the history entry instead of stacking one per click. The career tab also has a year filter, and both halves of that report are scoped to the same year — the endpoint filters by `date_applied` year and the client-side tiles use the same slice, so the tiles cannot disagree with the funnel
...
- **Job Search widget**: one card for the whole job hunt — headline tiles (total, active, offers, no response, response rate, average days to interview and to offer, stale in stage), the application funnel, a watch list, outcome chips, and location / age / offer-rate breakdowns. It replaces the six widgets that reported the same numbers three different ways (Total Applications, Active Pipeline, Outcomes Summary, No Response, Pipeline Breakdown, Timeline Analytics) plus the standalone Application Funnel section, so each figure now appears exactly once. Saved widget selections containing any of the retired ids migrate to `job_search` rather than emptying out
