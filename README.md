...
- **Per-stage staleness context**: the funnel shows the median days each stage typically takes, and every watch-list row says how far past normal *for its own stage* it is (`93 days past the 6d typical for this stage`) rather than only how long it has been waiting. Stages with too little history are left uncompared
