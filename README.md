...
- **The role picker is in the URL, beside the year.** `?role=google` names the role by company, resolved within the selected year, so the title is only added when one employer holds two roles in that same year; anything unrecognised, or no param at all, falls to the default role.
