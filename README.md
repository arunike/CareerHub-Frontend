...
- **Raise History flags a base that has moved underneath it.** A raise stores the pay it started from, so correcting the role's base afterwards leaves the two disagreeing; the modal says so and offers to re-chain the entries from the current figure.
...
- **A paycheck a raise cannot reach asks the same way.** Editing one paycheck's deductions used to pin its gross, so a later raise could never re-rate it and nothing on screen said so. The same prompt now names both rates, tags every affected paycheck by its pay date — each tag opens that paycheck, switching to the paycheck view from the year one — and releases those pins alone on **Use the new value** — a gross typed on any other paycheck is left as it is.
