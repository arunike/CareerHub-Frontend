...
- **The year ledger hides columns instead of scrolling sideways.** The register used to be an antd `Table` with `scroll={{ x: 1500 }}`, so reading Take-home on a laptop meant dragging the table 500px to the right and every column below `lg` was off screen. Columns now appear by priority as the width allows — Date, Take-home and Actual always; Gross and Tax from `sm`; the editable date picker from `md`; Pre-tax, Match and Notes from `xl`; the paycheck number and Tax rate from `xxl` — and nothing overflows at any width, including the tightest real case, a 1024px viewport with the 260px sidebar open, which leaves the card just 708px
- **Gross and take-home are both edited in place; the payslip modal is gone.** `Record actual payslip` opened a six-field dialog for gross, federal, state, Social Security, Medicare and take-home. Production told the story: across 42 recorded paychecks, **every one of the four per-line tax fields was empty** — nobody had ever filled one — while gross carried 24 values and take-home 42. The four columns were dropped from the database, the modal was deleted, and the two figures people actually record are now `LedgerCell` editors in the ledger row. Because no tax line can be recorded any more, `balanceTaxes` always scales federal and state together to reconcile with a recorded take-home, rather than holding a hand-entered line fixed
- **Experience earnings come from the Income tab's ledger, not a second calculation.** The breakdowns used to estimate a year's pay as annual rate × days worked, which ignores how often you are paid, when the bonus actually lands, the vesting schedule and any payslip you recorded — so the Experience page and the Income page quoted different numbers for the same year. `useLedgerEarnings` now runs the same `summarizeYears` the Income tab runs and the Experience views read its per-year, per-role figures, counting only paychecks whose date has passed. The old estimate survives as a fallback for a role the ledger knows nothing about, and each year says which it used ("26 paychecks" against "243 of 365 days"). The Income tab counts the same way: its year card was summing every modelled paycheck including the months still ahead, so the two pages disagreed for the current year. Gross, tax, deductions, take-home and match are all paid-to-date now, the header reads "17 of 26 paychecks paid", and the full-year figure survives as a hint on Gross rather than as the headline. The per-role summary card at the top of the page counted differently again — it showed every paycheck in the role's window, so a role ending in September showed one more paycheck than the year card below it — and now shares the same `totalsToDate` filter. The role card spells the sum out under the figure — "Gross = salary $114,230.77 + bonus and equity $24,750.00" — because gross being larger than salary is exactly what made two correct numbers look like a mismatch. **How gross is worked out** also splits its total by role, not just each component, because "Salary paid" is base pay only and reading it as a role's whole pay makes correct figures look wrong — the role's gross now appears verbatim under the total
- **The role card's earnings chip says what it means.** It was printing the current annual package under the words *total earnings*, so a role held for over a year read "$239,750 total earnings" when it had actually paid $420,000. The chip and the breakdown modal now read one helper, `totalEarned`, so they cannot disagree; a role with no start date has no period to total and is labelled "a year" instead
- **The pay breakdown shows its working, and the overall one is grouped by year and role.** The headline is money earned, not a rate: the year is cut into one stretch per pay rate and each is priced separately, printed under **How this adds up** — "1 Feb – 30 Jun · 150 days at $239,750 a year = $98,527", "1 Jul – 1 Oct · 93 days at $258,725 a year = $65,922", totalling $164,449 over 243 of 365 days. The current rate sits on its own labelled line above it rather than buried in a paragraph. The card covers every year of the role, not only the current one, so a job held since 2025 totals both. The overall full-time breakdown used to add up every tracked role's current annual package — including roles left years ago, which is not a figure that means anything. It now opens on an all-time total, splits pay into **Base Salary / Bonus / Equity**, and lists the roles that made up each one ("Base Salary $460,909 — Google $243,740, Netflix $217,169"). There is one donut, and each part of it is cut into the roles that made it up — base salary is one block of blue divided between employers, bonus one block of green, each in shades of its own colour — so you can see at a glance that most of the bonus came from one job. Hovering a slice says so outright ("Bonus · Google · 81% of bonus"), and a stacked bar plus a value list repeat the same split under each heading. Every figure is rounded once at source and only whole dollars are added, so the roles under a heading sum exactly to it and the headings sum exactly to the total; rounding each independently had them disagreeing by a dollar. A single role's card does the same, splitting each part by year. A role that cannot be priced is named under the chart with the reason rather than quietly left out — a job whose linked offer was saved blank used to vanish from the totals while still being counted in the "across N roles" header. A year filter narrows every section at once while the all-time total stays put. It is full-time only; internships have their own breakdown, which now splits the same way — Regular Pay and Overtime Pay, each divided between the internships that earned it, so a stint that never worked overtime simply does not appear under it. Both views share one panel, so the chart, bar and list behave identically
- **The pay breakdown follows your raises instead of the figure the role was created with.** Saving a raise writes only to `raise_history` — `base_salary` on the role is never touched — so **Pay Structure Breakdown** kept showing the pre-raise package indefinitely. When a raise landed during the year the headline becomes what the year has actually paid — each rate weighted by the days it ran — and the Base / Bonus / Equity rows below it split the same way, so the parts still add up to the total. The current rate does not disappear; it moves to a line underneath ("Now on $258,725 a year"). Without a raise the card is unchanged, still the plain annual rate. The weighting window is the stretch you actually held the role — it opens on your start date, closes on your leaving date, and for a role you are still in it stops at **today**, because the rest of the year has not been paid yet. The card names the window it used under the headline ("1 Feb – 1 Oct, the part of the year you have been paid for"), so the figure is checkable: joining on 1 Feb and being raised on 1 Jul gives 93 of 243 days at the higher rate, which is $164,449 rather than the $258,725 annual rate. Weighting across all twelve months instead would credit a leaver with months of a raise they never received, and would book a current role's pay for months that have not happened; the same raise reads $249,315 that way against $164,449 measured honestly. A plain average of before and after is wronger still — a December raise is worth about a twelfth of the increase, not half
- **A recorded raise re-rates every paycheck after its effective date.** A role carried one flat `base_salary`, so logging a mid-year raise changed the headline figure and nothing else — every paycheck in the year, including the ones paid before the raise, was grossed at the same rate. `Income/raiseSchedule.ts` turns the raise history into a step schedule (pay before the first raise is what that raise was raised *from*, not today's figure) and the ledger grosses each paycheck at the rate in force on its own pay date. A manual per-paycheck override still wins; the schedule only moves the default. It also works when the role record still stores the old salary, since the steps come from the raise entries rather than the headline figure
- **The raise reasons cover why pay actually moves.** The list was Merit, COLA, Market Adjustment, Retention, Other — which left no home for a **promotion**, the most common reason pay jumps at all, and led with `COLA`, an acronym you have to already know. It now reads Promotion, Merit increase, Market adjustment, Cost of living, Retention offer, Equity refresh, Role change, Pay correction, Other, and each one carries a line in the dropdown explaining when to pick it ("Raised to keep you from leaving, often against an outside offer"). The old values are all still in the list, because `raise_history` is a JSON blob with no server-side enum and a retired value would render as a blank tag on entries already saved
- **The raise form shows Before rather than asking for it.** Before is history — the pay this raise moved away from — so it renders as a dashed, muted, read-only figure, which is also what makes it obvious at a glance which side you can type in. The two columns were also drifting: After carries a `$`/`%Δ` toggle and sometimes a hint line, and each of those pushed its row further out of step with Before, 14px by the first row and 44px by the third. The form is now one row per figure with Before and After paired inside it, so the columns cannot drift however the After side grows — measured at 0px on all three rows, with and without a hint showing
- **An outbound link is sanitised before it reaches `href`.** A job link or meeting link is typed by the user, and job links also arrive from a Google Sheets import, so `javascript:alert(...)` in that field would run in this origin when clicked. `utils/safeUrl.ts` allows only http, https and mailto, treats a bare domain as https, and returns `undefined` for anything else so the anchor renders inert. Applied to the applications table, the application drawer and the event modal — the table's link was also missing `rel="noreferrer"` alongside its `target="_blank"`
- **Analytics widgets are placed by dragging, not by a fixed grid.** Both dashboards used a CSS grid with fixed column spans and dnd-kit reordering, so a short card such as the Watch List left a band of dead space beside its taller neighbour and there was no way to move it there. They now run on `WidgetGrid` (`react-grid-layout` v2) over 12 columns: drag a widget by its handle to any free spot and it stays exactly where it was dropped, or drag its right edge to change its width. Nothing compacts, so a gap you leave is a gap you meant; a drop onto cells another widget already occupies is refused rather than shoving that widget aside. The spot the widget will land in is outlined as you drag — the library's own placeholder is a red block, which reads as an error, so it is restyled to the same card in dashed outline. Heights are measured from each card's content, so a card is as tall as what is in it, and the untouched default settles every widget against the bottom of its own columns — a short card's neighbour starts directly below it rather than waiting for the tallest card in the row, so the first paint has no holes in it. Position and width persist per device in `localStorage`, and a phone falls back to one full-width column with dragging off
- **Every analytics widget folds to its header.** A chevron beside the title hides the body and drops the card to `h-auto`, so a folded widget is a header strip rather than an empty box, and the grid reclaims the rows underneath. Which widgets are folded is per-device state, like the collapsed sidebar, so it lives in `localStorage` rather than the account. `SectionCard` (job hunt) and `CollapsibleCard` (availability) read it from `WidgetCollapseProvider` through context, so no widget component needed a new prop
- **Roles and tax years can be switched off in Settings.** The Income pickers listed every role you have ever held and every year any of them covered, which for an old side role or a year before you started tracking is noise you cannot dismiss. **Settings → Organize → Income roles and years** lists both with a switch each, saved to `hidden_income_roles` and `hidden_income_years` on your account, so the choice follows you between devices. Hiding is display only: no pay data is deleted, and a hidden role's paychecks still count toward the year totals
...
- **The performance year offers only years the role was held, and defaults to one of them.** The picker always listed `taxYear − 2`, `− 1` and `taxYear`, and the default was blindly the year before — so a role that began in the year being modelled was prorated against a year it did not exist, and the target silently came out **$0.00** with no explanation beyond a note telling you to change the year yourself. `performanceYearOptions` derives the list from the role's start and end dates, and `resolvePerformanceYear` keeps the year-before when the role covers it and otherwise falls back to the nearest year it does. A stored year the role no longer covers is dropped rather than honoured
- **Proration shows its arithmetic.** It read `This role covered 59% of 2025`, which states the answer and hides the sum. It now reads `Held 214 of 365 days in 2025, so $30,000.00 × 58.6% = $17,589.04`, with the day counts coming from `prorationDetail` rather than being re-derived in the component. Switched off, it says how many days were worked anyway, so the number you are declining to prorate is still visible
- **The deferral base is a choice of three, not a toggle.** `Compute on base pay only` was a switch that carved out stipends and allowances — which framed gross pay, the normal case, as the exception, and gave no way at all to say what most plans actually do: exclude the **bonus**. It is now `Deferred and matched on`, with `All pay`, `Salary and bonus` (leaves out stipends and allowances) and `Salary only` (leaves out the bonus too), each option describing itself in the dropdown. The line beneath quotes real money — `$26,600.00 of this year's pay could be carved out`, or what the current choice keeps out — and when the year holds no allowances or bonus it says so, because then the setting cannot change a figure at all. `DEFERRAL_BASE_LABELS` and `excludedFromDeferral` live beside the ledger that applies them, and the match is computed on the same base as the deferral
- **A payout row says what each box is.** The schedule rows carried a date, a percent and an amount side by side with no labels, so `2026-03-13 · 100 % · $5,747.05` read as one run-on figure. Each field is now labelled (`Pay date` / `Paycheck`, `Share`, `Amount`) in a grid that drops to two columns on a phone, and the `With paycheck` / `Own date` switch is the shared `SegmentedToggle` rather than antd's — antd's track is the same tint as the panel it sits on, so the container vanished and the active pill floated on nothing, exactly as it did on the page-level switchers
- **One-off awards live in the Bonus tab, not under Allowances.** `Referral bonus` and `Spot bonus` were allowance presets, which put a single supplemental payment next to recurring stipends. They moved to `Extra bonuses`, where the label field is now a combobox over `BONUS_EXTRA_PRESETS` — referral, spot, sign-on, retention, exceeded expectations, project milestone, patent award, on-call, holiday and relocation. The presets carry a label and nothing else, because every one of them is taxed the same way as supplemental wages. An allowance already labelled `Referral bonus` is untouched; only the suggestion moved
- **The paycheck card's edit action sits with its other controls.** `Edit this paycheck's figures` had a bordered strip of its own below the take-home row, which read as a footer for a card that had already finished. It is now just `Edit` beside the prev/next arrows in the header, and the strip is gone — the card ends on its take-home line. The long label moves into the tooltip, which still distinguishes adjusting a paycheck from editing one already adjusted, and the button stays hidden for an off-cycle payment, which has no deductions to adjust
- **`Contributed in {year}` is a subtotal, and says so.** The block listed the year's projected traditional, Roth, match and total — the same $7,139.38 that made the gain read as a loss — with nothing marking it as a forecast. Every line is now the amount actually paid in, qualified in the header with `through Sep 30 · 17 of 26 paychecks`, and the projection moves to a quiet line beneath it (`On the current rate the year finishes at $12,942.22`). The qualifier disappears once the last paycheck of the year has been paid, so a finished year reads as a plain total. The limit bar above it stays a forecast — it is labelled `At this rate you finish the year…` and answers a different question
- **The gain is measured to date, not against the whole year.** The comparison subtracted every contribution the year would eventually make from a balance recorded today, so a mid-year account reported a loss it had never suffered — $5,689.66 of growth against $7,139.38 "paid in" read as −$1,449.72 when only about half that money had actually been deferred. `summarizeRetirement` and `retirementPerformance` now count only paychecks whose pay date has passed (`contributedToDate`), and the row says how many paychecks that is. The year's full projection is still kept separately, because the deferral-limit bar is a forecast and wants it. A row with no pay date counts as landed: treating an unknown date as future would zero out real contributions, which is the same wrong comparison pointing the other way
- **Unsaved work says so, and asks before you leave it.** The indicator was grey text hidden below `sm`, so a phone gave no sign at all that anything was pending. It is now a shared `UnsavedChangesActions` — an amber pill with a pulsing dot, a real Discard button and Save — used by Income and available to any page with a save. Leaving a page with unsaved changes now prompts: `useUnsavedChanges(isDirty, label)` registers the page with a provider in `Layout`, and both of the shell's navigation paths (sidebar menu, mobile quick actions) ask before routing. A `beforeunload` listener covers refresh and tab close, which no in-app guard can catch. The app uses `BrowserRouter` rather than a data router, so react-router's `useBlocker` is unavailable — hence the guard lives on the navigation call sites
- **401(k) performance reads on the year card, not just in the tab.** Recording an opening and a closing balance has always produced an investment gain — balance change less every dollar paid in, contributions and match together — but it only appeared at the bottom of the 401(k) tab, behind the One-paycheck view, so it was easy to miss entirely. The figure now sits under the year's 401(k) bar (`retirementPerformance` in `yearSummary.ts`), with the arithmetic stated in a line beneath it and a prompt in its place when no balances are recorded yet
  - **A role only counts once both of its balances are in.** Each employer's plan is its own account, so the balances add across roles — unlike the deferral limit, which follows the person. But pairing one role's closing balance against *every* role's contributions would report a loss that is really a missing number, so a role without both balances is left out along with its contributions, and the card says how many payrolls were excluded
...
- **No scrollbar inside the card.** The ledger used to be a 460px `scroll={{ y }}` box, which put a second scrollbar inside a card that already sits on a scrolling page — the ugliest thing on the view, and it hid how long the year actually was. The table now runs its full height with a `sticky` header, so the columns stay readable as the page scrolls and there is nothing nested to drag. Dropping `scroll.y` also drops antd's implicit `table-layout: fixed`, so the table sets it explicitly and keeps the column widths deterministic — without that, two phone columns silently grew to fit their content
- **The editor is the figure turning typable.** Clicking a value used to open a bordered `InputNumber` with a focus ring, and on a phone the 44px control floor made it taller than the row and shoved everything below it. It is now borderless on a transparent background with a 2px underline where the dashed hint was, right-aligned in the same weight and tabular figures as the value it replaced, and exempt from the touch floor since the cell you already tapped was the target. Its step handles are off (`controls={false}`): antd parks them over the right edge, which is exactly where a right-aligned figure sits, and a $100 step is no use against a payslip — turning them off also hands the field back the width antd was reserving for them. The `$` moved out of antd's prefix slot — which strands it at the far left of a right-aligned field, next to a placeholder that already had one — and into the formatter, so the field reads `$3,935.91` exactly like the figure did
- **The whole cell is the hit target, and it says so.** The first pass made only the digits clickable — a 69×19px target with a hover tint that appeared once you had already found it. The `<td>` now carries the click, so the target is 116×47px on a desktop and 101×41px on a phone, about four times the area and taller than the 44px touch minimum. The affordance is the figure, not a box around it: at rest an editable number carries a dashed underline, and hovering anywhere in the cell turns that underline solid and darkens the number, over a wash a step past the row's own hover tint so the extent of the target still reads. A bordered panel and a pencil icon were the first attempt and looked like a spreadsheet selection — 26 rows of them was noise. Every part of it is drawn on the cell or on the text, so none of it costs a pixel of column width
- **One gutter for every column.** antd's small table leaves 8px, the card-edge overrides added 12px, and the editable cells were adding their own on top, so no two columns were spaced alike. Every cell is now 12px from `md` and 8px below it, with 16px against the card edge — uniform within a breakpoint, which is what the eye actually compares. Widening the gutters left the 104px date picker in a 100px box and the 19px checkbox in a 12px one, so those two columns grew to match
- **The figure is the input; there is no second column for it.** `TAKE-HOME` and `ACTUAL` were the same number in two columns — both read `actual_net`, so a recorded paycheck printed its value twice, side by side, and the phone had to give up Gross to fit the duplicate. Take-home is now the editable cell (`LedgerCell`): click a figure and it swaps for an amount field seeded with what you recorded, with the modelled number greyed behind it so you can still see what the model expected; Enter or clicking away commits, Escape cancels, and clearing it goes back to the modelled figure. Nothing was migrated because nothing moved — the column that vanished never had a field of its own
- **The phone keeps the column you actually type into.** Recording what landed is the thing you do on a phone, and since take-home is now the field itself, no column has to give way for it — a phone shows date, gross and take-home, with a caret that expands it in place to the pay date, the note, and the figures its columns cannot show. The tap expands and never navigates, because switching to the paycheck view would unmount the list and destroy the panel it just opened. A `low` flag moves under the date on those widths rather than disappearing with the Gross column it normally sits beside
- **One accent, and it means something.** A row used to carry sky pre-tax, rose tax, violet match and four colours of antd `Tag`, so nothing read first. Every figure is now slate, deductions are written `−$365.63` so the sign carries the meaning rather than the colour, Take-home is the only bold figure, and emerald is reserved for a figure you recorded yourself — so a glance says which numbers are real and which the model produced. The `off` / `moved` / `adj` chips share one quiet style and only `low`, which flags a take-home the gross cannot support, keeps a warning tint
- **An allowance can be a one-off, not just a stipend.** A referral bonus, a home-office setup payment or a relocation allowance happens once, on one paycheck — the old model only offered per paycheck, per month and per year, so the nearest fit was "per year" landing on the first or last cheque. The `one time` frequency takes the paycheck it lands on (`#7 · Mar 27`), pays the whole amount there and nothing anywhere else, and hides `Times paid`, which cannot mean anything for a payment that happens once. If the named paycheck is no longer in the year — the schedule changed, or the pay dates moved — it falls back to the first or last paycheck rather than quietly paying nothing. The row summary reads `$1,500.00 once` instead of averaging it into a per-paycheck slice, which read as recurring money
- **The allowance label offers the common ones.** The field is now a combobox: type anything, or pick from work-from-home, internet, phone, wellness, car and commuter (monthly), meal and on-call (per paycheck), and referral, spot, home-office, relocation, learning-and-development and tuition (one time). Choosing one fills the label and the frequency, and resets `Times paid` so a stale multiplier cannot silently multiply the payment. **It deliberately does not set the tax treatment.** Commuter benefits and tuition assistance are excludable only up to annual caps this app does not model, so pre-selecting `Tax-free` would overstate take-home for anyone above the cap without saying so — the treatment stays on `Taxable` with its hint, one dropdown away
